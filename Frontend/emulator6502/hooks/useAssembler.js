const useAssembler = () => {
    ...
    function startAssembly( instructionCode ) {
    ...
    function LoadProgram(instructionCode) {
        var exampleValue = instructionCode;
        if (exampleValue.indexOf('\r\n') >= 0) {
            codesrc = exampleValue.split('\r\n');
        }
        else if (exampleValue.indexOf('\r') >= 0) {
            codesrc = exampleValue.split('\r');
        }
        else {
            codesrc = exampleValue.split('\n');
        }
    ...
    function getIdentifier(s, idx, stripColon) {
        ...
        var l = Math.min(end - idx, identMaxLength);
        ...
        for (var i = 0; i < stack.length; i++) {
            ...
            switch (item.type) {
                ...
                case 'paren':
                    ...
                    if (result < 0 || result > 255) isWord = true;
                    op = '';
                    break;
                case 'op':
                    op = item.v;
    ...
    function asmPass() {
        ...
        function logLine(excludeComments) {
            var s;
        ...
        while (sym) {
            ...
            if (
                ...
                if (instrtab[identRaw.replace(/\.\w+$/, '')] == null) {
                    ...
                    if (pass == 1) {
                        symtab[ident] = { 'v': pc, 'isWord': false, 'pc': pc };
                        hasLabel = ident;
                    }
                    sym.splice(ofs, 1);
                    c0 = sym[ofs].charAt(0);
                    if ((!bbcMode && sym[ofs] === '.OPT') || (bbcMode && sym[ofs] === 'OPT')) {
                        logLine();
            ...
            if (pragma) {
                ...
                if (bbcMode && pragma == ']') {
                    ...
                    (!bbcMode && (pragma == 'WORD' || pragma == 'DBYTE' || pragma == 'DBYT' ||
                        ((pragma == 'BYTE' || pragma == 'BYT') && sym.length >= 2 && sym[1].charAt(0) != '"'))) ||
                    ...
                    if (sym.length >= 2) {
                        ...
                        for (var j = 0; j < args.length; j++) {
                            ...
                            if (pass == 2) {
                                ...
                                var lb = (v >> 8) & 0xff;
                                var rb = v & 0xff;
                                if (pragma == 'WORD' || pragma == 'EQUW') { // big endian
                                    compile(pc, rb);
                                    compile(pc + 1, lb);
                                    asm = getHexByte(rb) + ' ' + getHexByte(lb);
                                    pict = (bbcMode ? 'EQUW' : '.WORD') + ' ' + hexPrefix + getHexWord(v);
                                }
                            ...
                            logLine();
                            pc += (pragma == 'BYTE' || pragma == 'BYT' || pragma == 'EQUB') ? 1 : 2;
